        ...
        $qid=$_GET["qid"];
            ...
            if($res)
            {
                ...
                while ($row = mysqli_fetch_assoc($res)) { 
                    echo $i.". ".$row["qs"]."<br>";
                    echo "<input type=\"radio\" value=\"".$row["op1"]."\" name=\"ans".$i.$j."\">".$row["op1"]."<br>";
                   ...
                   // echo "<input type=\"radio\"value=\"".($j+4)."\" name=\"ans".$i.$j."\">".$row["answer"]."<br><br>";  
                    $i++;                            
                }
                echo "<input id=\"btn\" type=\"submit\" name=\"submitans\" value=\"submit\"><br><br><br>";
                echo "</form><br><br>";
            }
            }
            else
            ...
            if(isset($_POST["submitans"])){
                global $score;
				$sql ="select answer from questions where quizid={$qid}";
				$res=$conn->query($sql);
				$row = $res->fetch_assoc();
				//echo '<pre>'; print_r($row); echo '</pre>';
				//echo "<script>alert(\"uuuuuuuuu scored ".$row['answer']." out of ".$count."\");</script>";
				//alert($sql);
				//$radioVal = $_POST["MyRadio"];
                for($i=1;$i<=$count;$i++)
                {
					//echo "<script>alert(\"uuuuuuuuu scored ".$_POST["ans".$i.$j]." out of ".$count."\");</script>";
